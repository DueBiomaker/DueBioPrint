...
namespace BioPrint.model
{
    public class GCodeGenerator
    {
        ...
        private double ReadD(string name, double def)
        {
        ...
        private void WriteD(string name, double v)
        {
            string sv = v.ToString("0.0000", GCode.format);
            printer.SetValue(name, sv);
        }

        private double ReadI(string name, int i)
        {
            return (int)printer.GetValue(name, i);
        }

        private void WriteI(string name, double v)
        {
            string sv = v.ToString("0.0000", GCode.format);
            printer.SetValue(name, sv);
        }
        ...
        public void WriteDouble(double val, int dig)
        {
            ...
            {
                case 0:
                    code.Append(val.ToString("0", GCode.format));
                    return;

                case 1:
                    code.Append(val.ToString("0.0", GCode.format));
                    return;

                case 2:
                    code.Append(val.ToString("0.00", GCode.format));
                    return;

                case 3:
                    code.Append(val.ToString("0.000", GCode.format));
                    return;

                case 4:
                    code.Append(val.ToString("0.0000", GCode.format));
                    return;

                case 5:
                    code.Append(val.ToString("0.00000", GCode.format));
                    return;

                default:
                    code.Append(val.ToString("0.000000", GCode.format));
                    return;
            }
        }
